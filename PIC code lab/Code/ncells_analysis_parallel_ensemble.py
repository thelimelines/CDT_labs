...
def run_simulation(ncells, seed):
...
results = Parallel(n_jobs=-1)(
    delayed(run_simulation)(ncells, seed)
    for ncells in ncells_range
    for seed in range(1, trials + 1)
)

# Aggregate results by ncells
aggregated_results = {}
for r in results:
    ncells = r["ncells"]# Extract the number of cells for this simulation
    # Initialize storage for this ncells value if not already present
    if ncells not in aggregated_results:
        aggregated_results[ncells] = {
            "simulation_time": [],
            "rms_noise": [],
            "omega": [],
            "omega_uncertainty": [],
    ...
    aggregated_results[ncells]["simulation_time"].append(r["simulation_time"])
    aggregated_results[ncells]["rms_noise"].append(r["rms_noise"])
    aggregated_results[ncells]["omega"].append(r["omega"])
    aggregated_results[ncells]["omega_uncertainty"].append(r["omega_uncertainty"])
    aggregated_results[ncells]["damping_rate"].append(r["damping_rate"])
    aggregated_results[ncells]["damping_uncertainty"].append(r["damping_uncertainty"])

# Compute means and uncertainties for aggregated data
ncells_values = []              # List of unique ncells values
...
for ncells, data in aggregated_results.items():
    ncells_values.append(ncells) 
